...
struct market_list {
	struct SaleInf inf;
	struct market_list *nextl, *nextr;
};
...
struct SaleInf read_data();
struct market_list *create_list(struct market_list *L, struct market_list *R);
struct market_list *add_sort(struct market_list *L, struct market_list *R, struct SaleInf data);
struct market_list *load_data(struct market_list *L, struct market_list *R);
struct market_list *free_data(struct market_list *L, struct market_list *R);
struct market_list *del_elem(struct market_list *L, struct market_list *R);
struct market_list *edit_elem(struct market_list *L, struct market_list *R);
int save_data(struct market_list *L, struct market_list *R);
void endBranchStock(struct market_list *L, struct market_list *R);
void endFactoryStock(struct market_list *L, struct market_list *R);
void endReport(struct market_list *L, struct market_list *R);
void print_list(struct market_list *L, struct market_list *R);
void delay() {
...
int main() {
	...
	while(1) {
		...
		puts(" 4. Edit element.");
		puts(" 5. Ending Branch Stock.");
		puts(" 6. Ending Factory Stock.");
		puts(" 7. Ending Stock Report.");
		puts(" 8. Save in file.");
		puts(" 9. Load from file.");
		puts(" 0. Exit.");
		puts("\n Choose key from 1 to 9, 0 - to exit");
		c = getch();
		switch(c) {
			case '1':
				L = create_list(L, R);
...
struct SaleInf read_data() {
	...
	data.marketNum = safeIntInput("\n Write Market Number >> ");
	data.sectionNum = safeIntInput("\n Write Section Number >> ");
	data.chequeNum = safeIntInput("\n Write Cheque Number >> ");
	data.productArticle = safeIntInput("\n Write Section Number >> ");
	data.productCount = safeIntInput("\n Write Section Number >> ");
	scanf("%s", &data.productName);
    data.productCost = safeFloatInput("\n Write Product Cost >> ");
	puts("\n Data recorded.");
	return data;
}

void endBranchStock(struct market_list *L, struct market_list *R) {
    struct market_list *t = L;
    int factoryNum;
...
struct market_list *add_sort(struct market_list *L, struct market_list *R, struct SaleInf data) {
	struct market_list *t1, *t2;
	t2 = (struct market_list *) malloc(sizeof(struct market_list));
	t2->inf = data;
	if(L == NULL) {
		t2->nextl = NULL;
		t2->nextr = L;
...
struct market_list *load_data(struct market_list *L, struct market_list *R) {
	...
	struct market_list *t1 = (struct market_list *) malloc(sizeof(struct market_list));
	long len = 0;
	while(fread(&t1->inf, sizeof(t1->inf), 1, f)) {
		struct market_list *t2 = (struct market_list *) malloc(sizeof(struct market_list));
		t2->inf = t1->inf;
		if (L == NULL) {
	    ...
	    t1->nextr = (struct market_list *) malloc(sizeof(struct market_list));
		t1 = t1->nextr;
	}
	free(t1);
	fclose(f);
