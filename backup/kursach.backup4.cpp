...
struct bidir_list *search_elem(struct bidir_list *L, struct bidir_list *R) {
	...
	if (foundL != NULL) {
        ...
        selectedIndex = safeIntInput("\n Выберите индекс элемента для работы (0 - чтобы вернуться) >> ");;
		if(selectedIndex == 0) {
			return NULL;
		}
	    selectedNode = foundL;
...
//	}
	...
	while(1) {
		...
		puts("\n 1. Завод");
		puts(" 2. Филиал.");
		puts(" 3. Ответственное лицо.");
		puts(" 4. Начальные ценности.");
		puts(" 5. Полученные ценности.");
		puts(" 6. Выбывшие ценности.");
		puts(" 7. Редактировать элемент целиком.");
		puts(" 8. Удалить элемент.");
		puts(" 0. Выход.");
		puts("\n Введите число от 1 до 7, 0 - чтобы выйти");
		c = getch();
		switch(c) {
...
void print_list(struct bidir_list *L, struct bidir_list *R) {
    ...
    while (1) {
        ...
        printf(" Страница %d из %d (ниже - S, выше - W, выход - Q)\n", currentPage, (totalElements + pageSize) / pageSize);
...
int save_data(struct bidir_list *L, struct bidir_list *R) {
	...
	while(1) {
		...
		if(d == 0) {
	    	...
	    	fclose(f);
	    	puts("\n Данные загружены в текстовый файла.");
	    	return 0;
		} else {
			puts("\n Ошибка ввода, попробуйте ещё раз.");
		}
